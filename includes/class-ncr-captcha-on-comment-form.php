	...
	public function uncr_redirect_fail_captcha_comment( $location, $comment ) {
		...
		if ( ! empty( $this->error ) ) {
			...
			if ( array_key_exists( 'empty_captcha', $this->error->{'errors'} ) ) {
				...
				wp_die( wp_kses_post( $this->error->{'errors'}['empty_captcha'][0] ) ); // captcha field is empty (not checked)

			} else if ( array_key_exists( 'invalid_captcha', $this->error->{'errors'} ) ) {

				wp_die( wp_kses_post( $this->error->{'errors'}['invalid_captcha'][0] ) ); // captcha is invalid ( failed verification )
			}
		}
